        ...
        switch(comandoSemPrefixo){
            ...
            case "nblk":
                try{
                    ...
                    if(blista_numero == numero_bot) return await socket.responderTexto(c, id_chat, comandos_info.grupo.nblk.msgs.bot_erro , mensagem)
                    else if(admins.includes(blista_numero)) return await socket.responderTexto(c, id_chat, comandos_info.grupo.nblk.msgs.admin_erro , mensagem)
                    let blista_grupo_lista = await grupos.obterListaNegra(id_grupo)
                    if(blista_grupo_lista.includes(blista_numero)) return await socket.responderTexto(c, id_chat, comandos_info.grupo.nblk.msgs.ja_listado, mensagem)
                    await grupos.adicionarUsuarioListaNegra(id_grupo, blista_numero)
                    await socket.responderTexto(c, id_chat, comandos_info.grupo.nblk.msgs.sucesso, mensagem)
            ...
            case "unblk":
                try{
                    ...
                    if(!dlista_grupo_lista.includes(dlista_numero)) return await socket.responderTexto(c, id_chat, comandos_info.grupo.unblk.msgs.nao_listado, mensagem)
                    await grupos.removerUsuarioListaNegra(id_grupo, dlista_numero)
                    await socket.responderTexto(c, id_chat, comandos_info.grupo.unblk.msgs.sucesso, mensagem)
                } catch(err){
                    throw err
                }
                break
            ...
            case 'antlink':
                try{
                    ...
                    if (estadoNovo) {
                        ...
                        await socket.responderTexto(c, id_chat, comandos_info.grupo.antlink.msgs.ligado, mensagem)
                    } else {
                        await grupos.alterarAntiLink(id_grupo, false)
                        await socket.responderTexto(c, id_chat, comandos_info.grupo.antlink.msgs.desligado, mensagem)
                    }
                } catch(err){
                    throw err
                }
            ...
            case "marcaroschupacabra":
                try{
                    if (!usuario_admin) return await socket.responderTexto(c, id_chat, comandos_info.outros.permissao.apenas_admin , mensagem)
                    if(!args.length) return await socket.responderTexto(c, id_chat, erroComandoMsg(comando, botInfo) , mensagem)
                    let qtdMensagem = texto_recebido
                    if(isNaN(qtdMensagem)) return await socket.responderTexto(c, id_chat, comandos_info.grupo.marcaroschupacabra.msgs.erro_qtd , mensagem)
                    if(qtdMensagem < 1 || qtdMensagem > 50) return await socket.responderTexto(c, id_chat, comandos_info.grupo.marcaroschupacabra.msgs.limite_qtd, mensagem)
                    if(!grupo.contador.status) return await socket.responderTexto(c, id_chat, comandos_info.grupo.marcaroschupacabra.msgs.erro_contador, mensagem)
                    ...
                    if(qtdInativos > 0){
                        ...
                        let inativosResposta = criarTexto(comandos_info.grupo.marcaroschupacabra.msgs.resposta_titulo, qtdMensagem, qtdInativos)
                        inativosResposta += `═════════════════\n╠\n`
                        for(let usuario of usuariosInativos){
                            inativosResposta += criarTexto(comandos_info.grupo.marcaroschupacabra.msgs.resposta_itens, usuario.id_usuario.replace(/@s.whatsapp.net/g, ''), usuario.msg)
                            mencionarUsuarios.push(usuario.id_usuario)
                        }
                        inativosResposta += `╠\n╚═〘 ${nome_bot?.trim()}® 〙`
                        await socket.enviarTextoComMencoes(c, id_chat, inativosResposta, mencionarUsuarios)
                    } else {
                        await socket.responderTexto(c, id_chat, comandos_info.grupo.marcaroschupacabra.msgs.sem_inativo, mensagem)
                    }
                } catch(err){
                    throw err
                }
